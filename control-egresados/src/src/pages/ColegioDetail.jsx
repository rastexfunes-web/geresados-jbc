                  ...
                  <tr key={a.id} className="clickable" onClick={() => navigate(`/alumnos/${a.id}`)}>
                    <td>
                      ...
                      <div style={{ fontSize: 12, color: "var(--slate)" }}>
                        {[a.tipoPrenda, a.telefono].filter(Boolean).join(" · ")}
                      </div>
                    </td>
...
const PRENDAS = ["Remera", "Chomba", "Campera", "Buzo"];
...
function NuevoAlumnoModal({ colegio, onClose, onCreated }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      await crearAlumno(
        { colegioId: colegio.id, nombre, apellido, apodo, dni, telefono, tipoPrenda },
        colegio
      );
      onCreated();
    } finally {
            ...
            <div className="chip-group">
              {PRENDAS.map((p) => (
                <button
                  key={p}
                  type="button"
                  className={`chip ${tipoPrenda === p ? "selected" : ""}`}
                  onClick={() => setTipoPrenda(tipoPrenda === p ? "" : p)}
                >
                  {p}
                </button>
              ))}
